...
SDLU_LoadIniRW(SDL_RWops* rwops, int freesrc)
{
  ...
  i = 0;
  do {
    if (buf[i] == '\0' && mode != ReadingPairValue) {
      break;
    } else if ((buf[i] == ';') && (mode != ReadingPairValue)) {
      //printf("Found comment, ignoring rest of line\n");
      mode = Reset;
    } else if (mode == Begin) {
      //printf("Begin\n");
      firstlen = 0;
      ...
      if (buf[i] == '[') {
        //printf("Reading new section\n");
        mode = ReadingSectionName;
      }
      else if (buf[i] != '=' && buf[i] != ' ' && buf[i] != '\n' && buf[i] != '\0') {
        //printf("Reading new key value pair\n");
      ...
      if (buf[i] == '\n') {
        //printf("Syntax error - EOL, resetting\n");
        mode = Reset;
      }
      else if (buf[i] == '=') {
        //printf("Read key name %s\n", first);
      ...
      if (buf[i] == '\n' || buf[i] == ';' || buf[i] == '\0') {
        char *value, *property;
        property = (char*)SDL_malloc(firstlen+1);
        SDL_strncpy(property, first, firstlen+1);
        if (secondlen == 0) {
          //printf("Empty value\n");
          ...
          //printf("Read value %s\n", second);
          value = (char*)SDL_malloc(secondlen+1);
          SDL_strncpy(value, second, secondlen+1);
      ...
      if (buf[i] == '\n') {
        //printf("Syntax error reading section name, resetting\n");
        mode = Begin;
      }
      else if (buf[i] == ']') {
        SDL_free(current_section);
        current_section = (char*)SDL_malloc(firstlen+1);
        SDL_strncpy(current_section, first, firstlen+1);
        mode = Reset;
        //printf("Switch to new section %s\n", current_section);
      }
      else {
        first[firstlen++] = buf[i];
      }
